...
func TestRuntimeStartsServesHealthAndStopsAgainstPostgres(t *testing.T) {
	...
	if dsn == "" {
		if os.Getenv("REQUIRE_INTEGRATION") == "1" {
			t.Fatal("required real PostgreSQL TEST_WORKER_DATABASE_URL is not configured")
		}
		t.Skip("real PostgreSQL runtime test not run: TEST_WORKER_DATABASE_URL is required")
	}
