...
Propose an eight-hour absolute session, fifteen-minute protected-request idle timeout, no persistent provider refresh tokens, secure opaque browser cookies and transactional local revocation. Provider-side revocation is asynchronous and cannot inherit the local immediate-revocation claim. Unknown assurance never satisfies step-up. See the complete [session policy, threat review and test plan](../architecture/P0.4-IDENTITY-DESIGN.md) before approval; these concrete values and trade-offs are not approved defaults yet.
